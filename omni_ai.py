...
# --- [ إعدادات الهوية البصرية - CYBER PRESTIGE ] ---
console = Console()
PRIMARY_BLUE = "#00F0FF"
...
# --- [ قراءة المفاتيح من متغيرات البيئة - SECURE CONFIG ] ---
OPENAI_KEY = os.environ.get("OPENAI_KEY")
GEMINI_KEY = os.environ.get("GEMINI_KEY")
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN")

if not all([OPENAI_KEY, GEMINI_KEY, TELEGRAM_TOKEN]):
    raise EnvironmentError("❌ Missing one or more required environment variables: OPENAI_KEY, GEMINI_KEY, TELEGRAM_TOKEN")

# تهيئة المحركات العصبية
...
# --- [ منطق البوت - BOT LOGIC ] ---
orchestrator = OmniOrchestrator()

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    log_status(f"Link Established with: {user.first_name} [ID: {user.id}]", style=PRIMARY_BLUE)

    welcome_text = (
        "⚡ **OMNI-AI SYSTEM ACTIVATED** ⚡\n\n"
        "مرحباً بك يا Ahmed في النواة المركزية.\n\n"
        "النظام يعمل الآن بكامل طاقته عبر:\n"
        "• **GPT-4o:** للتحليل البرمجي والمنطقي.\n"
        "• **Gemini 1.5:** للسرعة الفائقة والبيانات الضخمة.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log_status(f"Request from {update.effective_user.first_name} has been synchronized.")

    except Exception as e:
        log_status(f"CRITICAL SYSTEM ERROR: {str(e)}", style=ACCENT_CRIMSON)
        await status_msg.edit_text("❌ حدث انقطاع في النواة المركزية.")

# --- [ التشغيل - RUNNER ] ---
if __name__ == '__main__':
    log_status("OMNI-AI CORE IS BOOTING UP...", style=PRIMARY_BLUE)
